...
import { discussionGuides, glossary, modes, roles, topics } from './data';
...
export default function DebatePlatform() {
  ...
  const topicGuide = active ? discussionGuides[active] : discussionGuides[1];
  ...
  const stepLabels=topicGuide.map(step=>step.title);
  ...
  return <main>
    ...
    {topic && <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label={topic.title}><section className="workspace guided-workspace">
      ...
      <div className="guided-brief"><div><small>{topic.area}</small><h3>«{topic.thesis}»</h3><p>{topic.anchor}</p></div><div className="guided-progress" aria-label={`${complete} von 6 Schritten abgeschlossen`}><strong>{complete}<span>/6</span></strong><div>{stepLabels.map((label,index)=><i className={readiness[index]?'done':index===currentStep?'active':''} key={label}/>)}</div></div></div>
      <div className="workspace-grid guided-grid">
        <aside className="workspace-nav guided-nav"><small>{topic.title}</small>{stepLabels.map((label,index)=>{const unlocked=index===0||readiness.slice(0,index).every(Boolean);return <button className={index===currentStep?'active':readiness[index]?'done':''} disabled={!unlocked} onClick={()=>setCurrentStep(index)} key={label}><span>{readiness[index]?'✓':String(index+1).padStart(2,'0')}</span>{label}<i>{!unlocked?'gesperrt':index===currentStep?'jetzt':''}</i></button>})}</aside>
        <div className="workspace-content guided-content">
          <div className="potential-card"><div><small>{topicGuide[currentStep].title}</small><p>{topicGuide[currentStep].potential}</p></div><aside><p>{topicGuide[currentStep].discovery}</p></aside></div>

          {currentStep===0&&<section className="work-card guided-step" id="position"><div className="step-title"><span>01</span><div><small>{topic.area}</small><h3>{topicGuide[0].title}</h3></div></div><p className="do-this">{topicGuide[0].action}</p><div className="range-labels"><b>{topic.poles[0]}</b><b>{topic.poles[1]}</b></div><input aria-label={`Gewichtung zwischen ${topic.poles[0]} und ${topic.poles[1]}`} type="range" min="0" max="100" value={work.position} onChange={e=>update({position:+e.target.value,pre:+e.target.value})}/><output>{work.position}</output><label>{topicGuide[0].discovery}<textarea value={work.reason} onChange={e=>update({reason:e.target.value})} placeholder={topicGuide[0].starter}/></label><GuidedFooter ready={currentReady} step={currentStep} evidence={work.reason} nextLabel={topicGuide[1].title} onBack={()=>setCurrentStep(currentStep-1)} onNext={goNext} success={topicGuide[0].success} pending={topicGuide[0].pending}/></section>}

          {currentStep===1&&<section className="work-card guided-step" id="fragen"><div className="step-title"><span>02</span><div><small>{topic.area}</small><h3>{topicGuide[1].title}</h3></div></div><p className="do-this">{topicGuide[1].action}</p><div className="question-choices">{topic.questions.map((q,i)=><button className={work.questionIndex===i?'active':''} onClick={()=>update({questionIndex:i,questionResponse:work.questionIndex===i?work.questionResponse:''})} key={i}><small>{q.level}</small><span>{q.text}</span></button>)}</div>{customQuestion&&<button className={`custom-question choice ${work.questionIndex===topic.questions.length?'active':''}`} onClick={()=>update({questionIndex:topic.questions.length,questionResponse:''})}><b>Frage der Lehrperson:</b> {customQuestion}</button>}<label>{work.questionIndex===null?topicGuide[1].discovery:work.questionIndex<topic.questions.length?topic.questions[work.questionIndex].text:customQuestion}<textarea value={work.questionResponse} onChange={e=>update({questionResponse:e.target.value})} placeholder={topicGuide[1].starter}/></label><GuidedFooter ready={currentReady} step={currentStep} evidence={work.questionResponse} reference={work.questionIndex===null?'':work.questionIndex<topic.questions.length?topic.questions[work.questionIndex].text:customQuestion} nextLabel={topicGuide[2].title} onBack={()=>setCurrentStep(currentStep-1)} onNext={goNext} success={topicGuide[1].success} pending={work.questionIndex===null?`Wähle eine der sechs Fragen zu «${topic.title}».`:topicGuide[1].pending}/></section>}

          {currentStep===2&&<section className="work-card guided-step" id="beleg"><div className="step-title"><span>03</span><div><small>{topic.chapter}</small><h3>{topicGuide[2].title}</h3></div></div><p className="do-this">{topicGuide[2].action}</p><label>Szene innerhalb von {topic.chapter}<input type="text" value={work.scene} onChange={e=>update({scene:e.target.value})} placeholder={topic.anchor}/></label><div className="two-fields"><label>Im Wortlaut feststellbar<textarea value={work.observation} onChange={e=>update({observation:e.target.value})} placeholder={topicGuide[2].starter}/></label><label>{topicGuide[2].discovery}<textarea value={work.interpretation} onChange={e=>update({interpretation:e.target.value})} placeholder={topicGuide[2].starter2}/></label></div><GuidedFooter ready={currentReady} step={currentStep} evidence={`${work.observation} ${work.interpretation}`} reference={work.scene} nextLabel={topicGuide[3].title} onBack={()=>setCurrentStep(currentStep-1)} onNext={goNext} success={topicGuide[2].success} pending={!work.scene.trim()?`Verorte die Szene innerhalb von ${topic.chapter}.`:topicGuide[2].pending}/></section>}

          {currentStep===3&&<section className="work-card guided-step" id="perspektive"><div className="step-title"><span>04</span><div><small>{topic.perspectives.map(item=>item.name).join(' · ')}</small><h3>{topicGuide[3].title}</h3></div></div><p className="do-this">{topicGuide[3].action}</p><div className="perspective-card"><small>{topic.perspectives[perspective].name}</small><h4>{topicGuide[3].discovery}</h4><p>{topic.perspectives[perspective].prompt}</p></div><button className="secondary-button" onClick={()=>{setPerspective((perspective+1)%topic.perspectives.length);update({perspectiveResponse:''})}}>Andere Figur ↻</button><label className="response-field">{topic.perspectives[perspective].name} antwortet auf «{topic.thesis}»<textarea value={work.perspectiveResponse} onChange={e=>update({perspectiveResponse:e.target.value})} placeholder={topicGuide[3].starter}/></label><GuidedFooter ready={currentReady} step={currentStep} evidence={work.perspectiveResponse} reference={topic.perspectives[perspective].name} nextLabel={topicGuide[4].title} onBack={()=>setCurrentStep(currentStep-1)} onNext={goNext} success={topicGuide[3].success} pending={topicGuide[3].pending}/></section>}

          {currentStep===4&&<section className="work-card guided-step" id="diskussion"><div className="step-title"><span>05</span><div><small>{topic.counters[counter]}</small><h3>{topicGuide[4].title}</h3></div></div><p className="do-this">{topicGuide[4].action}</p><div className="counter guided-counter"><small>{topicGuide[4].discovery}</small><p>{topic.counters[counter]}</p><button onClick={()=>{setCounter((counter+1)%topic.counters.length);update({counterResponse:''})}}>Zweites Gegenargument ↻</button></div><label className="response-field">Deine Antwort auf «{topic.counters[counter]}»<textarea value={work.counterResponse} onChange={e=>update({counterResponse:e.target.value})} placeholder={topicGuide[4].starter}/></label><details className="discussion-toolbox"><summary>Gesprächsform für «{topic.title}»</summary><div className="mode-tabs">{modes.map((m,i)=><button className={mode===i?'active':''} onClick={()=>selectMode(i)} key={m.name}>{m.name}<small>{m.minutes} Min.</small></button>)}</div><div className="debate-tools"><div className="timer"><small>{modes[mode].name}</small><strong>{time}</strong><div><button onClick={()=>setRunning(!running)}>{running?'Pause':'Start'}</button><button onClick={()=>setSeconds(modes[mode].minutes*60)}>Neu beginnen</button></div></div><ol>{modes[mode].steps.map(s=><li key={s}>{s}</li>)}</ol></div></details><GuidedFooter ready={currentReady} step={currentStep} evidence={work.counterResponse} reference={topic.counters[counter]} nextLabel={topicGuide[5].title} onBack={()=>setCurrentStep(currentStep-1)} onNext={goNext} success={topicGuide[4].success} pending={topicGuide[4].pending}/></section>}

          {currentStep===5&&<section className="work-card guided-step" id="reflexion"><div className="step-title"><span>06</span><div><small>{work.pre} → {work.post}</small><h3>{topicGuide[5].title}</h3></div></div><p className="do-this">{topicGuide[5].action}</p><div className="vote-shift"><label>{topic.poles[0]} / {topic.poles[1]} · zuerst <input type="range" min="0" max="100" value={work.pre} disabled/><b>{work.pre}</b></label><span className={work.post-work.pre>0?'right':work.post-work.pre<0?'left':''}>{work.post-work.pre>0?'+':''}{work.post-work.pre}</span><label>{topic.poles[0]} / {topic.poles[1]} · jetzt <input type="range" min="0" max="100" value={work.post} onChange={e=>update({post:+e.target.value})}/><b>{work.post}</b></label></div><label>{topicGuide[5].discovery}<textarea value={work.reflection} onChange={e=>update({reflection:e.target.value})} placeholder={topicGuide[5].starter}/></label>{currentReady&&<div className="argument-chain"><small>{topic.title}</small><p><b>{topic.poles[0]} / {topic.poles[1]}:</b> {work.reason}</p><p><b>{work.scene}:</b> {work.interpretation}</p><p><b>{topic.counters[counter]}:</b> {work.counterResponse}</p><p><b>{work.pre} → {work.post}:</b> {work.reflection}</p></div>}<GuidedFooter ready={currentReady} step={currentStep} evidence={work.reflection} reference={`${topic.poles[0]} / ${topic.poles[1]} · ${work.pre} → ${work.post}`} onBack={()=>setCurrentStep(currentStep-1)} onNext={goNext} success={topicGuide[5].success} pending={topicGuide[5].pending}/><div className="actions"><button onClick={()=>window.print()}>Drucken</button><button onClick={()=>download('md')}>Markdown</button><button onClick={()=>download('json')}>JSON</button><button className="danger" onClick={reset}>Zurücksetzen</button></div></section>}
        </div>
      </div>
    </section></div>}
...
function GuidedFooter({ready,step,evidence,reference,nextLabel,onBack,onNext,success,pending}:{ready:boolean;step:number;evidence?:string;reference?:string;nextLabel?:string;onBack:()=>void;onNext:()=>void;success:string;pending:string}){
  const excerpt=evidence?.trim().replace(/\s+/g,' ').slice(0,150);
  return <div className={`guided-feedback ${ready?'ready':'waiting'}`} aria-live="polite"><div><i>{ready?'✓':'→'}</i><p>{reference&&<small>{reference}</small>}{excerpt&&<em>«{excerpt}{evidence!.trim().length>150?'…':''}»</em>}<span>{ready?success:pending}</span></p></div><nav>{step>0&&<button aria-label="Zum vorherigen Gedanken" onClick={onBack}>←</button>}{step<5&&<button className="guided-next" disabled={!ready} onClick={onNext}>{nextLabel} →</button>}</nav></div>
}
