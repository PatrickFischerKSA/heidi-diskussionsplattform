...
export default function DebatePlatform() {
  ...
  const stepPotential=[
    {title:'Teste deine erste Gewissheit.',text:'Der Regler ist kein Quiz. Er hält fest, was du vor der genaueren Prüfung denkst. Gerade deine Unsicherheit ist wertvoll: Sie zeigt, an welcher Stelle der Roman deine Meinung noch verändern kann.',question:'Was müsste im Text auftauchen, damit du den Regler später verschiebst?'},
    {title:'Mach aus einer Meinung eine untersuchbare Behauptung.',text:'Eine Vertiefungsfrage zwingt dich, Folgen und Betroffene mitzudenken. So wird aus «Ich finde …» eine Aussage, die andere prüfen, bestreiten oder weiterführen können.',question:'Für welche Figur hätte deine Position eine konkrete Folge – und welche?'},
    {title:'Lass den Roman deiner Meinung widersprechen dürfen.',text:'Eine genaue Szene kann deine Position stützen, aber auch ihre Grenze zeigen. Wenn du Beobachtung und Deutung trennst, erkennst du, was wirklich im Text steht und was du selbst daraus folgerst.',question:'Welches Wort, welche Handlung oder Reaktion trägt deine Deutung tatsächlich?'},
    {title:'Entdecke den blinden Fleck deiner eigenen Sicht.',text:'Dieselbe Situation sieht anders aus, wenn eine Figur Freiheit gewinnt, Verantwortung trägt oder einen Preis bezahlt. Der Rollenwechsel macht sichtbar, was dein bisheriges Urteil übersehen hat.',question:'Was kann diese Figur sehen oder verlieren, das du bisher nicht berücksichtigt hast?'},
    {title:'Mache dein Urteil belastbarer statt nur lauter.',text:'Ein starkes Gegenargument ist kein Hindernis, sondern ein Belastungstest. Wenn du seinen berechtigten Kern aufnimmst, musst du deine Position nicht aufgeben – aber genauer begrenzen.',question:'Welchen Teil deiner Position kannst du behalten, welchen musst du verändern?'},
    {title:'Mach deine Denkbewegung sichtbar.',text:'Die Zahl vorher und nachher zeigt nicht, ob du «richtig» liegst. Sie zeigt, was die Textstelle, die fremde Perspektive und das Gegenargument mit deinem Urteil gemacht haben. Auch eine unveränderte Position braucht nun einen besseren Grund.',question:'Welcher einzelne Gedanke war für dein Schlussurteil entscheidend?'},
  ];
    ...
    {topic && <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label={topic.title}><section className="workspace guided-workspace">
      ...
      <div className="workspace-grid guided-grid">
        ...
        <div className="workspace-content guided-content">
          <div className="potential-card"><div><small>Worum es hier wirklich geht · Schritt {currentStep+1}</small><h3>{stepPotential[currentStep].title}</h3><p>{stepPotential[currentStep].text}</p></div><aside><small>Entdeckungsfrage</small><p>{stepPotential[currentStep].question}</p></aside></div>
