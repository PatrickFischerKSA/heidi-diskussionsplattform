...
export default function DebatePlatform() {
  ...
  return <main>
    ...
    {topic && <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label={topic.title}><section className="workspace guided-workspace">
      ...
      <div className="guided-brief"><div><small>Dein Auftrag</small><h3>Finde heraus, ob deine erste Meinung trägt.</h3><p>Du sollst keine «richtige» Antwort erraten. Du prüfst deine spontane Position am Roman, lässt sie von einer anderen Sicht herausfordern und beobachtest, wo dein Urteil genauer, stärker oder anders wird.</p></div><div className="guided-progress" aria-label={`${complete} von 6 Schritten abgeschlossen`}><strong>{complete}<span>/6</span></strong><div>{stepLabels.map((label,index)=><i className={readiness[index]?'done':index===currentStep?'active':''} key={label}/>)}</div></div></div>
      <div className="workspace-grid guided-grid">
        ...
        <div className="workspace-content guided-content">
          <div className="potential-card"><div><small>{topic.title} · Schritt {currentStep+1}</small><h3>{topicGuide[currentStep].title}</h3><p>{topicGuide[currentStep].potential}</p></div><aside><small>Entdeckungsfrage</small><p>{topicGuide[currentStep].discovery}</p></aside></div>

          {currentStep===0&&<section className="work-card guided-step" id="position"><div className="step-title"><span>01</span><div><small>Position beziehen</small><h3>Wo stehst du – und warum?</h3></div></div><p className="do-this"><b>Tu jetzt:</b> {topicGuide[0].action}</p><div className="range-labels"><b>{topic.poles[0]}</b><b>{topic.poles[1]}</b></div><input aria-label="Position von 0 bis 100" type="range" min="0" max="100" value={work.position} onChange={e=>update({position:+e.target.value,pre:+e.target.value})}/><output>{work.position}</output><label>Meine erste Begründung<textarea value={work.reason} onChange={e=>update({reason:e.target.value})} placeholder={topicGuide[0].starter}/></label><GuidedFooter ready={currentReady} step={currentStep} evidence={work.reason} onBack={()=>setCurrentStep(currentStep-1)} onNext={goNext} success={topicGuide[0].success} pending={topicGuide[0].pending}/></section>}

          {currentStep===1&&<section className="work-card guided-step" id="fragen"><div className="step-title"><span>02</span><div><small>Vertiefen</small><h3>Welche Frage trifft deine Position?</h3></div></div><p className="do-this"><b>Tu jetzt:</b> {topicGuide[1].action}</p><div className="question-choices">{topic.questions.map((q,i)=><button className={work.questionIndex===i?'active':''} onClick={()=>update({questionIndex:i,questionResponse:work.questionIndex===i?work.questionResponse:''})} key={i}><small>{q.level}</small><span>{q.text}</span></button>)}</div>{customQuestion&&<button className={`custom-question choice ${work.questionIndex===topic.questions.length?'active':''}`} onClick={()=>update({questionIndex:topic.questions.length,questionResponse:''})}><b>Frage der Lehrperson:</b> {customQuestion}</button>}<label>Meine Antwort<textarea value={work.questionResponse} onChange={e=>update({questionResponse:e.target.value})} placeholder={topicGuide[1].starter}/></label><GuidedFooter ready={currentReady} step={currentStep} evidence={work.questionResponse} reference={work.questionIndex===null?'':work.questionIndex<topic.questions.length?topic.questions[work.questionIndex].text:customQuestion} onBack={()=>setCurrentStep(currentStep-1)} onNext={goNext} success={topicGuide[1].success} pending={work.questionIndex===null?`Wähle eine der sechs Fragen zu «${topic.title}».`:topicGuide[1].pending}/></section>}

          {currentStep===2&&<section className="work-card guided-step" id="beleg"><div className="step-title"><span>03</span><div><small>Am Text prüfen</small><h3>Was zeigt eine konkrete Stelle?</h3></div></div><p className="do-this"><b>Tu jetzt:</b> {topicGuide[2].action}</p><label>Kapitel oder Situation<input type="text" value={work.scene} onChange={e=>update({scene:e.target.value})} placeholder={topic.chapter}/></label><div className="two-fields"><label>1 · Beobachtung<textarea value={work.observation} onChange={e=>update({observation:e.target.value})} placeholder={topicGuide[2].starter}/></label><label>2 · Deutung<textarea value={work.interpretation} onChange={e=>update({interpretation:e.target.value})} placeholder={topicGuide[2].starter2}/></label></div><p className="method-note">Prüfe den Unterschied: Die Beobachtung könnte man im Buch unterstreichen. Die Deutung erklärt, warum sie für deine Position wichtig ist.</p><GuidedFooter ready={currentReady} step={currentStep} evidence={`${work.observation} ${work.interpretation}`} reference={work.scene} onBack={()=>setCurrentStep(currentStep-1)} onNext={goNext} success={topicGuide[2].success} pending={!work.scene.trim()?`Verorte zuerst die Szene im angegebenen Textanker: ${topic.chapter}.`:topicGuide[2].pending}/></section>}

          {currentStep===3&&<section className="work-card guided-step" id="perspektive"><div className="step-title"><span>04</span><div><small>Perspektivkarte · Rolle wechseln</small><h3>Was würde diese Figur einwenden?</h3></div></div><p className="do-this"><b>Tu jetzt:</b> {topicGuide[3].action}</p><div className="perspective-card"><small>Du argumentierst als</small><h4>{topic.perspectives[perspective].name}</h4><p>{topic.perspectives[perspective].prompt}</p></div><button className="secondary-button" onClick={()=>{setPerspective((perspective+1)%topic.perspectives.length);update({perspectiveResponse:''})}}>Andere Perspektive ziehen ↻</button><label className="response-field">Antwort aus dieser Perspektive<textarea value={work.perspectiveResponse} onChange={e=>update({perspectiveResponse:e.target.value})} placeholder={topicGuide[3].starter}/></label><GuidedFooter ready={currentReady} step={currentStep} evidence={work.perspectiveResponse} reference={topic.perspectives[perspective].name} onBack={()=>setCurrentStep(currentStep-1)} onNext={goNext} success={topicGuide[3].success} pending={topicGuide[3].pending}/></section>}

          {currentStep===4&&<section className="work-card guided-step" id="diskussion"><div className="step-title"><span>05</span><div><small>Kuratiertes Gegenargument prüfen</small><h3>Was antwortest du darauf?</h3></div></div><p className="do-this"><b>Tu jetzt:</b> {topicGuide[4].action}</p><div className="counter guided-counter"><small>Gegenargument</small><p>{topic.counters[counter]}</p><button onClick={()=>{setCounter((counter+1)%topic.counters.length);update({counterResponse:''})}}>Anderes Gegenargument ↻</button></div><label className="response-field">Meine differenzierte Antwort<textarea value={work.counterResponse} onChange={e=>update({counterResponse:e.target.value})} placeholder={topicGuide[4].starter}/></label><details className="discussion-toolbox"><summary>Gesprächsmodus und Timer öffnen</summary><div className="mode-tabs">{modes.map((m,i)=><button className={mode===i?'active':''} onClick={()=>selectMode(i)} key={m.name}>{m.name}<small>{m.minutes} Min.</small></button>)}</div><div className="debate-tools"><div className="timer"><small>Timer</small><strong>{time}</strong><div><button onClick={()=>setRunning(!running)}>{running?'Pause':'Start'}</button><button onClick={()=>setSeconds(modes[mode].minutes*60)}>Zurücksetzen</button></div></div><ol>{modes[mode].steps.map(s=><li key={s}>{s}</li>)}</ol></div></details><GuidedFooter ready={currentReady} step={currentStep} evidence={work.counterResponse} reference={topic.counters[counter]} onBack={()=>setCurrentStep(currentStep-1)} onNext={goNext} success={topicGuide[4].success} pending={topicGuide[4].pending}/></section>}

          {currentStep===5&&<section className="work-card guided-step" id="reflexion"><div className="step-title"><span>06</span><div><small>Abschliessen</small><h3>Was hat sich bewegt?</h3></div></div><p className="do-this"><b>Tu jetzt:</b> {topicGuide[5].action}</p><div className="vote-shift"><label>Vorher <input type="range" min="0" max="100" value={work.pre} disabled/><b>{work.pre}</b></label><span className={work.post-work.pre>0?'right':work.post-work.pre<0?'left':''}>{work.post-work.pre>0?'+':''}{work.post-work.pre}</span><label>Nachher <input type="range" min="0" max="100" value={work.post} onChange={e=>update({post:+e.target.value})}/><b>{work.post}</b></label></div><label>Welches Argument hat deine Position verändert oder differenziert?<textarea value={work.reflection} onChange={e=>update({reflection:e.target.value})} placeholder={topicGuide[5].starter}/></label>{currentReady&&<div className="argument-chain"><small>Deine Argumentkette</small><p><b>Ausgangspunkt:</b> {work.reason}</p><p><b>Textprüfung:</b> {work.interpretation}</p><p><b>Gegenargument:</b> {work.counterResponse}</p><p><b>Schluss:</b> {work.reflection}</p></div>}<GuidedFooter ready={currentReady} step={currentStep} evidence={work.reflection} reference={`${work.pre} → ${work.post}`} onBack={()=>setCurrentStep(currentStep-1)} onNext={goNext} success={topicGuide[5].success} pending={topicGuide[5].pending}/><div className="actions"><button onClick={()=>window.print()}>Drucken</button><button onClick={()=>download('md')}>Markdown</button><button onClick={()=>download('json')}>JSON</button><button className="danger" onClick={reset}>Zurücksetzen</button></div></section>}
        </div>
      </div>
    </section></div>}
...
function GuidedFooter({ready,step,evidence,reference,onBack,onNext,success,pending}:{ready:boolean;step:number;evidence?:string;reference?:string;onBack:()=>void;onNext:()=>void;success:string;pending:string}){
  const excerpt=evidence?.trim().replace(/\s+/g,' ').slice(0,150);
  return <div className={`guided-feedback ${ready?'ready':'waiting'}`} aria-live="polite"><div><i>{ready?'✓':'→'}</i><p><b>{ready?'Dieser Gedanke trägt':'An diesem konkreten Punkt weiterarbeiten'}</b>{reference&&<small>Bezug: {reference}</small>}{excerpt&&<em>Aus deinem Beitrag: «{excerpt}{evidence!.trim().length>150?'…':''}»</em>}<span>{ready?success:pending}</span></p></div><nav>{step>0&&<button onClick={onBack}>← Zurück</button>}{step<5&&<button className="guided-next" disabled={!ready} onClick={onNext}>Weiter zu Schritt {step+2} →</button>}</nav></div>
}
