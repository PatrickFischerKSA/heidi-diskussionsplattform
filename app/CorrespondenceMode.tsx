...
const voiceFrames:Record<StudentCharacter,{focus:string;letter?:string;voice?:string}>={
  Clara:{focus:'Clara schreibt aus einem Alltag, in dem andere über ihren Körper, ihre Reise und ihre Freude sprechen. Ihre Briefe werden konkret, wenn sie zwischen eigenem Wunsch und fremder Erwartung unterscheidet.',letter:'Liebe Heidi, als du von … erzählt hast, musste ich an … denken. Hier in Frankfurt …'},
  Heidi:{focus:'Heidi verbindet Gedanken oft mit einer beobachteten Person, einem Tier, einem Geräusch oder einer Veränderung auf der Alp. Daraus entsteht ihre Frage an Clara oder Peter.',letter:'Liebe Clara, heute habe ich bei … gesehen, wie … Das erinnert mich an …',voice:'Clara / Peter, eben ist … passiert. Dabei habe ich gemerkt, dass …'},
  Peter:{focus:'Peter erklärt Gefühle selten ausführlich. Seine Stimme gewinnt Kontur, wenn eine knappe Beobachtung, eine Ausweichbewegung oder eine konkrete Tat Eifersucht, Scham oder Zuneigung hörbar macht.',voice:'Heidi, wegen … Ich habe … Nicht gesagt habe ich dir, dass …'},
};
...
export default function CorrespondenceMode({onClose}:{onClose:()=>void}){
  ...
  const voiceFrame=voiceFrames[character];
  ...
  return <div className="modal-backdrop correspondence-backdrop" role="dialog" aria-modal="true" aria-label="Heidi, Clara und Peter – gemeinsamer Austausch"><section className="correspondence-shell">
    ...
    {!credentials?<RoomGate onConnected={next=>{setCredentials(next);setStatus('Der gemeinsame Posttisch ist geöffnet.')}}/>:<div className="correspondence-workbench">
      ...
      <div className="message-stream student-stream">
        ...
        <section className="student-composer" aria-label="Eigene Nachricht verfassen"><header><div><small>Eigener Beitrag</small><h3>{character}s Stimme an diesem Posttisch</h3></div><span>{body.length}/1600</span></header><div className="composer-identity"><label>Kürzel<input value={alias} maxLength={32} onChange={event=>setAlias(event.target.value)} placeholder="z. B. L7"/></label><fieldset><legend>Figurenstimme</legend><div>{(['Clara','Heidi','Peter'] as StudentCharacter[]).map(item=><button type="button" className={character===item?'active':''} onClick={()=>chooseCharacter(item)} key={item}>{item}</button>)}</div></fieldset><fieldset><legend>Form</legend><div>{formats[character].map(item=><button type="button" className={channel===item?'active':''} onClick={()=>setChannel(item)} key={item}>{labels[item]}</button>)}</div></fieldset></div><div className="character-focus"><small>Was bei {character} im Text auf dem Spiel steht</small><p>{voiceFrame.focus}</p></div><label>{channel==='letter'?'Brieftext':'Gesprochener Text'}<textarea value={body} maxLength={1600} onChange={event=>setBody(event.target.value)} placeholder={voiceFrame[channel]}/></label>{body.trim().length>20&&<p className="draft-echo"><b>Der Faden hört in deinem Entwurf:</b> «{body.trim().replace(/\s+/g,' ').slice(0,130)}{body.trim().length>130?'…':''}» – die spätere Randstimme wird genau an dieser Formulierung und an der vorherigen Nachricht anknüpfen.</p>}{channel==='voice'&&<p className="voice-privacy">Die Stimme entsteht erst beim Abspielen im Browser. Es wird keine Schüler*innenstimme aufgenommen.</p>}<div className="composer-send"><span>{status}</span><button onClick={submit} disabled={busy||!alias.trim()||body.trim().length<3}>{busy?'Unterwegs …':'In den Postlauf geben'} <b>→</b></button></div></section>
      </div>
    </div>}
  </section></div>
}
