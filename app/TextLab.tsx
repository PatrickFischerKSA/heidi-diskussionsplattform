...
const quotePattern=/[«»„“\"]/;
type FeedbackResult={tone:'pause'|'next'|'ready';praise:string;action:string};
const observationFeedback=(value:string,hasMark:boolean)=>{
  if(!hasMark)return {tone:'pause',praise:'Du hast den Untersuchungsschritt geöffnet.',action:'Gehe zu Schritt 1 zurück und klicke genau einen Satz als Beleg an.'} as FeedbackResult;
  if(!value.trim())return {tone:'pause',praise:'Dein Belegsatz ist bereits gewählt.',action:'Kopiere daraus ein auffälliges Wort in «Anführungszeichen» und beschreibe, ob es wiederholt, gesteigert oder mit einem Körperzeichen verbunden wird.'} as FeedbackResult;
  if(value.trim().length<28)return {tone:'next',praise:'Du hast eine erste Beobachtung formuliert.',action:'Ergänze einen zweiten Satz: Benenne genau, wie das gewählte Wort im Satz verwendet wird – zum Beispiel wiederholt, verstärkt oder einer Figur zugeordnet.'} as FeedbackResult;
  if(!quotePattern.test(value))return {tone:'next',praise:'Du beschreibst bereits ein erkennbares Textsignal.',action:'Setze jetzt ein bis fünf Wörter aus der Passage in «Anführungszeichen». So wird sichtbar, worauf deine Beobachtung beruht.'} as FeedbackResult;
  return {tone:'ready',praise:'Das ist sorgfältige Textarbeit: Du benennst ein Signal und belegst es wörtlich.',action:'Lies deine Beobachtung einmal laut. Falls sie schon «zeigt» oder «bedeutet» sagt, verschiebe diesen Deutungsteil in den nächsten Schritt.'} as FeedbackResult;
};
const interpretationFeedback=(value:string)=>{
  if(!value.trim())return {tone:'pause',praise:'Deine Beobachtung bleibt oben sichtbar – du musst nicht neu anfangen.',action:'Beginne mit «Das zeigt, dass …» und ergänze genau eine Aussage über die Haltung zum Lesen, die Lernmethode oder die Wirkung des Lesens.'} as FeedbackResult;
  if(value.trim().length<38)return {tone:'next',praise:'Du hast eine mögliche Bedeutung angesetzt.',action:'Füge nach deiner Aussage «Dadurch …» ein und beschreibe in einem zweiten Satz die Wirkung auf Heidi, Peter oder die zuhörende Figur.'} as FeedbackResult;
  if(!/(weil|dadurch|deshalb|zeigt|verdeutlicht|wirkt|macht|indem)/i.test(value))return {tone:'next',praise:'Deine Deutung enthält schon eine Aussage über den Leseprozess.',action:'Verbinde sie nun ausdrücklich mit deiner Beobachtung: Setze vor die Wirkung «Dadurch» oder begründe sie mit «weil».'} as FeedbackResult;
  return {tone:'ready',praise:'Dir gelingt die Verbindung von Textsignal und Bedeutung.',action:'Unterstreiche gedanklich den Satzteil, der die Wirkung erklärt. Genau diesen Zusammenhang prüfst du jetzt an einem zweiten Satz.'} as FeedbackResult;
};
const checkFeedback=(value:string,counterMark:number|null)=>{
  if(counterMark===null)return {tone:'pause',praise:'Deine erste Deutung steht und kann jetzt geprüft werden.',action:'Klicke einen anderen Satz an. Nimm möglichst einen, in dem die Figur anders handelt oder eine andere Person spricht.'} as FeedbackResult;
  if(!value.trim())return {tone:'pause',praise:'Du hast einen zweiten Satz als Gegenprobe ausgewählt.',action:'Beginne mit «Dieser Satz bestätigt …» oder «Dieser Satz begrenzt …» und nenne danach einen konkreten Unterschied zum ersten Satz.'} as FeedbackResult;
  if(value.trim().length<35)return {tone:'next',praise:'Du vergleichst bereits zwei Stellen miteinander.',action:'Ergänze, was sich genau verändert: Wer spricht, wie reagiert die Figur oder welche Wirkung hat das Lesen jetzt?' } as FeedbackResult;
  if(!/(aber|jedoch|zugleich|nur|teilweise|andererseits|obwohl|bestätigt|begrenzt)/i.test(value))return {tone:'next',praise:'Deine Gegenprobe bezieht den zweiten Satz erkennbar ein.',action:'Setze zwischen Gemeinsamkeit und Unterschied eines dieser Wörter: «aber», «zugleich», «teilweise» oder «begrenzt».'} as FeedbackResult;
  return {tone:'ready',praise:'Stark geprüft: Du wiederholst deine Deutung nicht nur, sondern machst sie genauer.',action:'Lies nun im Ergebnis Beleg, Beobachtung, Deutung und Gegenprobe direkt nacheinander. Prüfe, ob jeder Teil auf den vorherigen antwortet.'} as FeedbackResult;
};
...
export default function TextLab({onClose}:{onClose:()=>void}){
  ...
  const isComplete=(id:string)=>{const value={...emptyEntry(),...state.entries[id]};return value.marks.length===1&&value.signalChoice===guides[id].answer&&observationFeedback(value.observation,true).tone==='ready'&&interpretationFeedback(value.interpretation).tone==='ready'&&checkFeedback(value.check,value.counterMark).tone==='ready'};
    ...
    {view==='demo'?<Demo onDone={()=>setView('lab')}/>:<>
      ...
      <div className="textlab-layout"><aside className="passage-rail"><small>{reader}s Leseweg</small>{passages.map((item,index)=><button aria-current={activeId===item.id?'step':undefined} className={activeId===item.id?'active':''} onClick={()=>changePassage(item.id)} key={item.id}><span>0{index+1}</span><div><small>{item.phase}</small><b>{item.title}</b></div><i>{isComplete(item.id)?'●':'○'}</i></button>)}<div className="lab-method"><b>Deine sechs Schritte</b>{steps.map((step,index)=><span className={index===microStep?'active':''} key={step}>{index+1} · {step}</span>)}</div></aside>
        <div className="lab-bench"><header><div><small>{active.phase} · {active.chapter}</small><h3>{active.title}</h3></div><span>{active.pdfPages}</span></header><p className="edition-note">Wortlaut der bereitgestellten Ausgabe; die Markierung verändert den Text nicht.</p>
          ...
          {microStep===0&&<section className="micro-card"><header><span>01</span><div><small>Orientieren</small><h4>Welcher Satz trägt die stärkste Spur?</h4></div></header><p>Lies alle Sätze. Wähle genau einen, an dem du deine Untersuchung beginnen möchtest.</p><div className="passage-text select-one">{active.sentences.map((sentence,index)=><button aria-pressed={entry.marks.includes(index)} className={entry.marks.includes(index)?'marked':''} onClick={()=>update({marks:[index],counterMark:entry.counterMark===index?null:entry.counterMark})} key={index}><span>{String(index+1).padStart(2,'0')}</span>{sentence}</button>)}</div><Feedback result={entry.marks.length===1?{tone:'ready',praise:'Gute Entscheidung: Du hast einen klaren Ausgangspunkt gesetzt.',action:'Lies den gewählten Satz noch einmal und zeige mit dem Finger auf das auffälligste Wort. Im nächsten Schritt vergleichst du mögliche Textsignale.'}:{tone:'pause',praise:'Du nimmst dir Zeit, bevor du deutest.',action:'Klicke jetzt genau den Satz an, der am meisten über das Lesen, Lernen oder Verstehen verrät.'}}/><button className="micro-next" disabled={entry.marks.length!==1} onClick={next}>Weiter zum Textsignal →</button></section>}
          ...
          {microStep===3&&<section className="micro-card"><header><span>04</span><div><small>Verknüpfen</small><h4>Was zeigt deine Beobachtung?</h4></div></header><div className="evidence-carry"><small>Deine Beobachtung</small><p>{entry.observation||'Noch keine Beobachtung formuliert.'}</p></div><p>Formuliere eine vorsichtige Deutung. Erkläre ausdrücklich, wie dein Textsignal mit dem Leseprozess zusammenhängt.</p><textarea value={entry.interpretation} onChange={event=>update({interpretation:event.target.value})} placeholder="Die Wendung zeigt, dass …, weil …"/><Feedback result={interpretationResult}/><button className="micro-next" disabled={interpretationResult.tone!=='ready'} onClick={next}>Weiter zur Gegenprobe →</button></section>}
          {microStep===4&&<section className="micro-card"><header><span>05</span><div><small>Differenzieren</small><h4>Hält deine Deutung dem restlichen Text stand?</h4></div></header><p>Wähle einen anderen Satz. Prüfe, ob er deine Deutung bestätigt, ergänzt oder begrenzt.</p><div className="counter-sentences">{active.sentences.map((sentence,index)=><button disabled={entry.marks.includes(index)} aria-pressed={entry.counterMark===index} className={entry.counterMark===index?'marked':''} onClick={()=>update({counterMark:index})} key={index}><span>{String(index+1).padStart(2,'0')}</span>{sentence}</button>)}</div><textarea value={entry.check} onChange={event=>update({check:event.target.value})} placeholder="Der zweite Satz bestätigt …, aber er zeigt zugleich …"/><Feedback result={checkResult}/><button className="micro-next" disabled={checkResult.tone!=='ready'} onClick={next}>Ergebnis ansehen →</button></section>}
          {microStep===5&&<section className="micro-card result-card"><header><span>06</span><div><small>Lesespur abgeschlossen</small><h4>Deine Deutung hat jetzt ein Fundament.</h4></div></header><div className="result-chain"><div><small>Beleg</small><p>{active.sentences[entry.marks[0]]||'–'}</p></div><div><small>Beobachtung</small><p>{entry.observation||'–'}</p></div><div><small>Deutung</small><p>{entry.interpretation||'–'}</p></div><div><small>Gegenprobe</small><p>{entry.check||'–'}</p></div></div><Feedback result={isComplete(activeId)?{tone:'ready',praise:'Du hast eine vollständige Lesespur gebaut: Beleg, Beobachtung, Deutung und Prüfung greifen ineinander.',action:'Öffne die nächste Passage. Suche dort zuerst nach einem Textsignal, das sich von deinem bisherigen unterscheidet.'}:{tone:'next',praise:'Mehrere Teile deiner Lesespur sind bereits angelegt.',action:'Klicke oben den ersten Schritt ohne Häkchen an und bearbeite genau den dort genannten nächsten Handgriff.'}}/></section>}
